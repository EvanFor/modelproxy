 ...
 * file so that the Proxy class can easily access the structure of the
 * configuration.
 * 
 * @author ShanFan
 * @created 24-3-2014
 ...
 * 
 * @param {String|Object}
 *            path The file path of inteface configuration or the interface object
 */
function InterfaceManager(path) {
	this._path = path;
	
	// {Object} Interface Mapping, The key is interface id and the value is a json profile for this interface.
	this._interfaceMap = {};
	
	// {Object} A interface Mapping for client, the key is interface id and the value is a json profile for this interface.
	this._clientInterfaces = {};
	
	// {String} The path of rulebase where the interface rules is stored. This value will be override if user specified the path of rulebase in interface.json.
...
InterfaceManager.prototype = {
	...
	_loadProfiles : function(profiles) {
		...
		// ------------------------------------
		this._restful = profiles.restful === "true" ;
		// ------------------------------------
		
		if (profiles.status === undefined) { throw new Error('There is no status specified in interface configuration!'); }
	...
	getRule : function(interfaceId) {
		if (!interfaceId || !this._interfaceMap[interfaceId]) { throw new Error('The interface profile ' + interfaceId + " is not found."); }
		path = this._interfaceMap[interfaceId].ruleFile;
		if (!fs.existsSync(path)) { throw new Error('The rule file is not existed.\npath = ' + path); }
		try {
