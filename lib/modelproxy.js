/**
f * ModelProxy As named, this class is provided to model the proxy.
 * 
 * @author ShanFan
 * @created 24-3-2014
 */
 ...
 *            profile. This profile describes what the model looks like. 
 *            eg:
 *            profile = { getItems: 'Search.getItems', getCart: 'Cart.getCart' }
 *            profile = ['Search.getItems', 'Cart.getCart'] profile =
 *            'Search.getItems' profile = 'Search.*'
 */
	...
	if (profile instanceof Array) {
		...
		for (var i = profile.length - 1; i >= 0; i--) {
			...
			if (!prof[methodName]) {
				...
				// The method name is duplicated, so the full interface id is set as the method name.
			} else {
	...
	for ( var method in profile) {
		this[method] = (function(methodName, interfaceId) {
			...
			return function(params) {
				...
				// Push this method call into request queue. Once the done method is called, all requests in this queue will be sent.
				this._queue.push({
					params : params,
					proxy : proxy
				});
 ...
 * 
 * @param {String}
 *            path The path refers to the interface configuration file.
 */
...
ModelProxy.Interceptor = function(req, res) {
	// todo: need to handle the case that the request url is multiple interfaces combined which configured in interface.json.
