/**
 * ProxyFactory, Proxy This class is provided to create proxy objects following
 * the configuration
 * 
 * @author ShanFan
 * @created 24-3-2014
...
Proxy.getMockEngine = function() {
	...
	this._mockEngine = require("../rivermock/" + this._engineName);
	return this._mockEngine;
};
...
// {Object} An object map to store created proxies. The key is interface id and the value is the proxy instance.
Proxy.objects = {};
...
Proxy.create = function(interfaceId) {
	...
	this.objects[interfaceId] = new this(opt);
	return this.objects[interfaceId];
};

Proxy.prototype = {
	request : function(params, callback, errCallback, cookie) {
		// if ( typeof callback !== 'function' ) { console.error( 'No callback function for request = ', this._opt ); return; }
		...
		if (self._opt.method === 'POST') {
			// ----------------------------
			options.headers['Content-Type'] = self._opt.dataType === "json" ? 'application/json;charset=UTF-8' : 'application/x-www-form-urlencoded';
			querystring = JSON.stringify(params);
			// 下面这句导致严重后果
			options.headers['Content-Length'] = self.strBtyeLength(querystring);
			// ----------------------------
		} else if (self._opt.method === 'GET') {
			// ----------------------------
			if (self._opt.restful) {
				for (var i = 0; i < self._opt.params.length; i++) {
					var k = self._opt.params[i];
					if (k in params) {
						options.path = options.path.replace(":" + k, params[k]);
					} else {
						if (options.path.indexOf("=") > 0) {
							options.path = options.path.replace(k + "=:" + k, "");
							console.error("Error: the value of param '" + k + "' is not provided, remove the param from the current url !");
						} else
							throw new Error("param '" + k + "' is must !");
					}
				}
			}
			// ----------------------------
		...
		var req = http.request(options, function(res) {
			...
			res.on('end', function() {
				...
				var result = {};
				try {
					result = self._opt.encoding === ENCODING_RAW ? buffer : (self._opt.dataType !== 'json' ? iconv.fromEncoding(buffer, self._opt.encoding) : JSON.parse(iconv.fromEncoding(buffer, self._opt.encoding)));
				} catch (e) {
					clearTimeout(timer);
	...
	strBtyeLength : function(str) {
		// 其实也很简单利用encodeURI编码单个字符，然后跟根据编码后的长度，获取字符串的byte长度。是根据页面的字符编码不同有所不同，utf-8一个汉字3byte，gb2312一个汉字2byte。
		var bytelen = 0;
		var chars = str.split('');
	...
	_mockRequest : function(params, callback, errCallback) {
		try {
			...
			if (Proxy._engineName === 'f6') {
				callback(engine.mock4spec(this._rule, reqoptions));
				return;
			}
